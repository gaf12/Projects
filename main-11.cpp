...
//The initField function initilizes the field with UNKOWN cells and then randomly sets some of the cells to be mine (using the setMine function)
//Input:
...
void initField(int num_of_mine) {
    ...
    // This for loop sets the whole field to unknown cells.
    for (int i = 0; i < 8; i++)
    {
        for (int j = 0; j < 5; j++)
        {
    ...
    // This code below sets 10 random cells to be mines..
    setMine(num_of_mine);
}
...
bool isMine(int i, int j) {
    // The if condition below checks if there is a mine or not in the cell..And returns true if
	 // the cell has a mine....
...
void reveal(int i,int j) {
	 ...
	 int mineCount = 0;
	 // The if conditions below checks if i and j are unknown.. If its unknown the preciding if statements goes on to
	 // check 8 cells around i and j, and counts the number of mines around i and j..
	 if (getCell(i,j) == UNKNOWN)
        {
				// Checking the upper most cell of cell i and j.. northwest.. below..
            if (((getCell(i - 1, j - 1) == UNFLAGGED_MINE) || (getCell(i - 1, j - 1) == FLAGGED_MINE))  && i - 1 >=0 && j - 1 >= 0)
            {
                mineCount = mineCount + 1;
            }
				// Checking the west of cell i and j...
            if (((getCell(i , j - 1) == UNFLAGGED_MINE) || (getCell(i, j - 1) == FLAGGED_MINE)) && j - 1 >= 0)
            {
                mineCount = mineCount + 1;
            }
				// Checking the southwest conner cell of cell i and j..
            if (((getCell(i + 1, j - 1) == UNFLAGGED_MINE) || (getCell(i + 1, j - 1) == FLAGGED_MINE)) && i + 1 <= 7 && j - 1 >= 0)
            {
                mineCount = mineCount + 1;
            }
				// Checking the north cell of cell i and j...
            if (((getCell(i - 1, j) == UNFLAGGED_MINE) || (getCell(i - 1, j) == FLAGGED_MINE)) && i - 1 >= 0)
            {
                mineCount = mineCount + 1;
            }
				// Cheking the northeast cell of cell i and j..
            if (((getCell(i - 1, j + 1) == UNFLAGGED_MINE) || (getCell(i - 1, j + 1) == FLAGGED_MINE)) && i - 1 >= 0 && j + 1 <= 4)
            {
                mineCount = mineCount + 1;
            }
				// Checking the east cell of cell i and j...
            if (((getCell(i, j + 1) == UNFLAGGED_MINE) || (getCell(i, j + 1) == FLAGGED_MINE)) && j + 1 <= 4)
            {
                mineCount = mineCount + 1;
            }
				// Checking the southeast cell of cell i and j....
            if (((getCell(i + 1, j + 1) == UNFLAGGED_MINE) || (getCell(i + 1, j + 1) == FLAGGED_MINE)) && i + 1 <= 7 && j + 1 <= 4)
            {
                mineCount = mineCount + 1;
            }
				// Checking south cell of cell i and j
            if (((getCell(i + 1, j) == UNFLAGGED_MINE) || (getCell(i + 1, j) == FLAGGED_MINE)) && i + 1 <= 7)
            {
                mineCount = mineCount + 1;
            }
            
				// The code below sets the cell i and j to the number of mines returned..
				// And that number is displayed at the location of  cell i and j....
	 ...
	 // The code below is my recursive function.. if the above i and j cell does not have a mine
	 // the code goes into the cells around it and keeps going until a bomb is close....
    if (mineCount == 0)
    {
		  if ( j + 1 <= 4)
		  {
...
void executeCmd(string cmd)
{
	 ...
	 // The first if condition below checks to see if the cmd.at(0) which is the first letter of cmd is f.. Then goes on to
	 // see if there is a mine at i and j or not.. if there is a mine, one is added to num_coorect_flagged_mine..
	 // And if there is no mine one is addedd to num_incorrect_flagged_mine.
    if (cmd.at(0) == 'f')
    {
		  if (getCell(i, j) == UNKNOWN)
		  {
				setCell(i, j, INCORRECT_FLAGGED_MINE);
				num_incorrect_flagged_mine = num_incorrect_flagged_mine + 1;
		  }
		  if (getCell(i, j) == UNFLAGGED_MINE)
		  {
				setCell(i, j, FLAGGED_MINE);
				num_correct_flagged_mine = num_correct_flagged_mine + 1;
		  }
    }
    
    // The code below checks to see if cmd.at(0) == r.. if true, i and j are passed into the reveal function
	 // and the if statement sets i and j to explode if there is a mine at i and j.
    else if(cmd.at(0) == 'r')
    {
	    reveal(i, j);
	 ...
	 // The else if statement below checks if cmd.at(0) == u, then it undos flagged mines or wrongly flagged mines back to its
	 // original state.
	 else if (cmd.at(0) == 'u')
	 {
		  //This if statemenet checks and sets wrongly flagged mine back to unknown
	    if(getCell(i, j) == INCORRECT_FLAGGED_MINE)
		 {
				setCell(i, j, UNKNOWN);
				num_incorrect_flagged_mine = num_incorrect_flagged_mine - 1;
		  }
		  // This if statement sets flagged mines back to unflagged mine..
